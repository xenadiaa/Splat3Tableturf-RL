...
from switch_connect.vision_capture.adapter import (
    FFmpegCaptureSource,
...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "height": 1080,
    "fps": 30,
    "out_dir": "switch_connect/vision_capture/debug",
    "warmup_seconds": 5.0,
    "prefix": "capture",
...
def main() -> int:
    ...
    p.add_argument(
        "--config",
        default="switch_connect/capture_config.json",
    ...
    try:
        ...
        _ = src.read_latest(timeout_seconds=max(0.1, float(cfg.get("warmup_seconds", DEFAULT_CONFIG["warmup_seconds"]))))
        ...
        while True:
            ...
            frame = src.read_latest(
                timeout_seconds=float(cfg.get("read_timeout_seconds", DEFAULT_CONFIG["read_timeout_seconds"]))
            )
            if frame is None:
